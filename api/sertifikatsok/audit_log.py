...
from sertifikatsok.revocation_info import RevocationInfoResponse
...
class RevocationInfoAuditLogger:
    def __init__(
        self,
        env: Environment,
        cert: str,
        ...
        self.env = env
        self.type = type
        self.results: RevocationInfoResponse | None = None

    def set_results(self, results: RevocationInfoResponse) -> None:
        self.results = results
    ...
    def __exit__(
        ...
        if value is not None or self.results is None:
            result = "ERROR"
        elif not self.results.cacheable:
            result = "PARTIAL"
        else:
            result = "OK"
