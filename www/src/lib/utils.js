...
export const getCompanies = async function (startOfName) {
  const queryParams = new URLSearchParams([["navn", startOfName]]);
  ...
  if (response._embedded && response._embedded.enheter) {
    for (let i = 0, len = response._embedded.enheter.length; i < len; i++) {
      companies[
        `${response._embedded.enheter[i].navn} ` +
          `(${response._embedded.enheter[i].organisasjonsnummer})`
      ] = null;
    }
  }
  return companies;
...
export const isBrregSearchable = function (query) {
  if (
    query.length < 4 ||
    query.startsWith("ldap://") ||
    query.startsWith("NTRNO-")
  ) {
    return false;
  }
...
export const callOcspApi = async function (cert) {
  //await new Promise((r) => setTimeout(r, 2000));

  let searchParams = new URLSearchParams(window.location.search);
  searchParams.append("cert", cert);

  let response = await fetch(`/revocation_info?${searchParams}`, {
    headers: { "sertifikatsok-version": getVersion() },
  });
